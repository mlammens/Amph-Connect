...
Record of analysis for *Rana sylvatica*.
----------------------------------------
...
results <- spocc::occ(query = "Rana sylvatica", from = "gbif", limit = 10000, has_coords = TRUE)
# retrieve data table from spocc object
results.data <- results[["gbif"]]$data[[formatSpName("Rana sylvatica")]]
...
```{r}
selCoords <- data.frame(x = c(-72.7744, -72.5383, -72.3983, -73.8316, -73.9963, -74.0677, -72.7744), y = c(41.2574, 41.833, 41.9761, 42.0169, 41.7777, 41.2842, 41.2574))
...
envs <- raster::getData(name = "worldclim", var = "bio", res = 0.5, lat = 41.628, lon = -73.267)
...
```{r}
writeRaster(x = proj, filename = "wood-frog.asc", overwrite = TRUE)
```
