...
Record of analysis for *Ambystoma maculatum*.
---------------------------------------------
...
results <- spocc::occ(query = "Ambystoma maculatum", from = "gbif", limit = 500, has_coords = TRUE)
# retrieve data table from spocc object
results.data <- results[["gbif"]]$data[[formatSpName("Ambystoma maculatum")]]
...
```{r}
selCoords <- data.frame(x = c(-73.3618, -73.4936, -71.3842, -72.0873, -73.3618), y = c(43.3635, 41.0197, 41.2183, 43.7139, 43.3635))
...
bgExt <- rgeos::gBuffer(occs.sp, width = 0.1)
```
...
```{r}
projCoords <- data.frame(x = c(-73.9687, -73.9591, -73.977, -73.9371, -73.966, -73.8836, -73.8905, -73.8877, -73.9262, -73.9289, -73.7531, -73.6378, -73.7215, -73.5087, -73.5636, -73.5444, -73.9687), y = c(-73.9687, -73.9591, -73.977, -73.9371, -73.966, -73.8836, -73.8905, -73.8877, -73.9262, -73.9289, -73.7531, -73.6378, -73.7215, -73.5087, -73.5636, -73.5444, -73.9687))
